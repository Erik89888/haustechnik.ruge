...
(function () {
  ...
  if (!reduce && hasIO) {
    each('.svc-hero-content h1, .page-head h1, .svc-head h2, .bereich-head h2, .section-header h2, .cta-section h2, .nibe-copy h2, .funding-inner h2', splitWords);
  }
  ...
  if (hasIO && !reduce) {
    var itemSel = '.svc-check-list li, .nibe-list li, .svc-facts li, .bereich-detail, .info-band .info-item, .footer-top > div, .topic-option, .contact-box, .steps li';
    var items = document.querySelectorAll(itemSel);
    each(itemSel, function (el) { el.classList.add('reveal-item'); });
  ...
  // Kontakt: Nachrichtenfeld wächst beim Schreiben mit (statt innen zu scrollen)
  each('#kontaktForm textarea', function (ta) {
    var grow = function () {
